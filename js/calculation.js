					...
					function declination (model) {
						...
						this.calculate = function (glat, glon, h, date) {
							if (unnormalizedWMM === undefined) {
								throw new Error("ÐÐµÑ‚ ÐºÐ¾ÑÑ„Ñ„Ð¸Ñ†Ð¸ÐµÐ½Ñ‚Ð¾Ð² WMM.")
							}
							if (glat === undefined || glon === undefined) {
								throw new Error("ÐÐµ Ð·Ð°Ð´Ð°Ð½Ñ‹ ÑˆÐ¸Ñ€Ð¾Ñ‚Ð° Ð¸Ð»Ð¸ Ð´Ð¾Ð»Ð³Ð¾Ñ‚Ð°.");
							}
...
//                              date = new Date (date);
			...
			var app = new Vue({
				...
				data: {
					...
					date: 0,
					success: true,
					mymap: L.map('mapid').setView([55, 55], 7),
				},
				created: function () {
					L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}', {
						...
						maxZoom: 18,
						id: 'mapbox.streets',
						accessToken: 'pk.eyJ1IjoiZG1seXJhZSIsImEiOiJjanY3emd1M28wYXdiNGVxeWIweDZwOG96In0.nWh-35IuWPcW7exzICpWaA'
					}).addTo(this.mymap);
					...
					this.longitude = -55;
					let now = new Date();
					this.date = now.getFullYear() + "-" + (now.getMonth() > 8 ? '' : '0') + (now.getMonth() + 1) + "-" + (now.getDate() > 8 ? '' : '0') + now.getDate();
				},
				computed: {
					result: function () {
						...
						if (typeof lat != "number") {
							...
							return result.toFixed(4) + '° ' + znak + ' (' + Math.floor(result) + '°' + Math.floor((result - Math.floor(result)) * 60) + '\''+ Math.floor((result*100 - Math.floor(result*100)) * 60) + '" '+ znak +')';
						}
					},
				},
				methods: {
